  ...
  return (
    <>
      <section className=" px-[65px] mt-[32px] ">
        <h1 className="mb-10 text-start text-4xl font-bold">
          Carrito de Compras
        </h1>
          ...
          {showCart ? (
            <ul>
              {cart.map((item) => (
                <li
                  key={item.id}
                  className="h-[272px] relative w-[680px] border-b border-t flex items-center gap-4 justify-start"
                >
                  ...
                  <div className="flex gap-x-24">
                    <div className="grid gap-14 justify-center">
                      <div className="flex flex-col text-sm gap-y-1.5">
                        <h4 className="text-black/80 font-medium">
                          {item.title}
                        </h4>
                        <p className="text-black/50 text-[15px] font-medium">
                          Talla: #{item.talla}
                        </p>
                        <p className="text-black/50 text-[15px] font-medium">
                          Precio: ${item.price}
                    ...
                    <div className="w-[252px] text-sm grid gap-y-14">
                      ...
                      <div className="flex gap-x-14 items-center">
                        ...
                        {item.clasificar === "popular" ? (
                          <p className="h-[22px] text-[9px] text-white bg-[#666] border inline-block px-8 font-bold ">
                            POPULAR
                          </p>
                        ) : null}
                        {item.clasificar === "descuento" ? (
                          <p className="h-[22px] text-[9px] text-[#666] border inline-block px-8 font-bold ">
                            DESCUENTO
                          </p>
                        ) : null}
                        {item.clasificar === "nuevo" ? (
                          <p className="h-[22px] text-[9px] text-[#666] border inline-block px-8 font-bold ">
                            NEW
                          </p>
                        ) : null}
          ...
          <div className="w-[468px] bg-white grid gap-y-1 h-[373px] text-[15px] border rounded-lg p-8 ">
            {showCart ? (
              <div>
                ...
                <button className="mt-6 w-full rounded-md bg-blue-500 h-[50px] font-medium text-blue-50 hover:bg-blue-600">
                  Check out
                </button>
              </div>
            ) : (
              <div className="flex flex-col items-center">
                <p className="text-black/80 text-center text-xl font-medium">
                  Agrega productos en la tienda para cotizar tus productos
                </p>
                <a
                  href="/es/shop"
                  className="mt-6 grid cursor-pointer place-content-center px-28 rounded-md bg-blue-500 h-[50px] font-medium text-blue-50 hover:bg-blue-600"
                >
                  Visitar Tienda
                </a>
              </div>
