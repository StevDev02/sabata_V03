...
export function NavBar() {
  ...
  useEffect(() => {
    if (search === "") {
      setFiler((prevState) => ({ ...prevState, estilo: "all" }));
    }
    ...
    if (
      ...
      setFiler((prevState) => ({ ...prevState, tipo: "zapatos" }));
    }
  }, [search]);
  ...
  return (
    <>
      <div
        id="filters"
        className="flex z-50 w-full px-4 h-10 justify-between items-center"
      >
        ...
        <div className="flex place-content-center gap-2">
          <input
            className="border pl-4 border-b-white hover:shadow-lg shadow-md w-[520px] py-1 "
          ...
          />
          <button
            className="w-[36px] border shadow grid place-content-center h-[36px] "
        ...
        <div className="flex items-center gap-x-2">
          ...
          >
            <Button
              disabled={loading}
              className="w-[150px] bg-transparent text-black disabled:cursor-not-allowed border py-2 rounded-sm border-zinc-500 hover:text-white hover:bg-black"
              type="submit"
            >
              {loading ? (
          ...
          <button
            className="w-[30px] cursor-pointer h-[30px] rounded-full shadow-md grid place-content-center "
            onClick={() => {
              setShowCart(true);
      ...
      <div className="w-full pb-5 text-center">
        ...
        <p className="text-sm poppins">
          ¡Bienvenida al vestidor oficial de ASOS! Viene cargado de vestidos{" "}
          <br /> para mujer con los que renovar tu estilo.
        </p>
      </div>

      {showCart && (
        <button
          className="z-[999] right-4 top-[45.5px] translate-x-[-50%] translate-y-[-50%] fixed -m-2 p-2 text-gray-400 hover:text-gray-500"
          onClick={() => setShowCart(false)}
          type="button"
        >
          <svg
            className="w-[28px] border rounded-full shadow-lg h-[28px] "
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            viewBox="0 0 24 24"
